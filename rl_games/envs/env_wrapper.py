...
import numpy as np
from rl_games.envs.sts2.environment import STS2Environment
...
class Env(object):
    ...
    def __init__(self, config_env, test=False, N_roles=None):
        """Set up variables
        ...
        self.env = STS2Environment(
            ...
            with_pygame=config_env['render'], # Turn on pygame feature, so that you can `render()`
            timeout_ticks=config_env['max_tick'])  # Max-length of one round of game
        self.render_ = config_env['render']
    ...
    def step0(self, actions_int, roles=None, measure=False):
        ...
        if self.self_play:
            ...
            # print(f"actions_int_home: {actions_int_home}")
            # print(f"actions_int_away: {actions_int_away}")
        ...
        self.env.render() if self.render_ else None
        self.env_step += 1
        self.control_team = self.state_json['control_team']
        self.control_index = self.state_json['control_index']
        self.update_puck_state()
        ...
        info['win_rate'] = np.mean(self.win_record) if self.win_record else 0
        obs = {}
        obs['states'] = state_home
        obs['states_op'] = state_away
        obs['obs'] = list_obs_home
        obs['obs_op'] = list_obs_away

        reward_dict = {}
        reward_dict['reward'] = reward
        reward_dict['local_rewards'] = local_rewards
