...
from zhikong.utils import reduce_reflex_angle_deg
...
class Reward(object):
    ...
    def assessment_reward(self) -> float:
        ...
        return sum(self.base_reward_elements)
...
class RewardComponent(ABC):
    ...
    @abstractmethod
    def get_potential(self, state: State, is_terminal, weight) -> float:
        ...
...
class NormalisedComponent(RewardComponent, ABC):
    ...
    def __init__(self,
        ...
        """
        self.name = name
        self.state_variable=state_variables
    ...
    def change_prop(self,prop):
        self.state_index_of_value = self.state_variable.index(prop)

    def change_target(self,numb):
        self.target=numb

    def change_weight(self,numb):
        self.weight=numb
    ...
    def calculate(self, state: State, prev_state: State, is_terminal: bool):
        ...
        if self.potential_difference_based:
            ...
            reward = self.get_potential2(state, prev_state,is_terminal, weight) #- self.get_potential(prev_state, False, weight)
        else:
            reward = self.get_potential(state, is_terminal, weight)
        return reward
...
class ErrorComponent(NormalisedComponent, ABC):
    ...
    def get_potential(self, state: State, is_terminal, weight) -> float:
        ...
        return weight*(1 - self._normalise_error(error))

    def get_potential2(self, state: State, prev_state: State, is_terminal, weight) -> float:
        if is_terminal and self.potential_difference_based:
            return self.POTENTIAL_BASED_DIFFERENCE_TERMINAL_VALUE
        ...
        if self.is_constant_target():
            ...
            # else we have to look it up from the state
            target = state[self.target_index]
        value = state[self.state_index_of_value]-prev_state[self.state_index_of_value]
        error = abs(value - target)
        return weight*(1 - self._normalise_error(error))
...
class AsymptoticErrorComponent(ErrorComponent):
    ...
    def __init__(self,
        ...
        """
        super().__init__(name, prop, state_variables, target, is_potential_based, weight)
