...
altitude_sl_ft = BoundedProperty('position/h-sl-ft', 'altitude above mean sea level [ft]', 2000, 15000)
...
lng_geoc_deg = BoundedProperty('position/long-gc-deg', 'c [deg]', -180, 180)
dist_travel_m = Property('position/distance-from-start-mag-mt', 'distance travelled from starting position [m]')
true_heading_rad=BoundedProperty('attitude/heading-true-rad','true-heading',0,2*math.pi)
heading_setpoint=BoundedProperty('ap/heading_setpoint','target heading',0,360)
altitude_setpoint=BoundedProperty('ap/altitude_setpoint','target altitude',0,15000)
# velocities
...
pid_aileron_cmd = BoundedProperty('ap/aileron_cmd', 'aileron commanded position, normalised', -1., 1.)

elevator_cmd = BoundedProperty('fcs/elevator-cmd-norm', 'elevator commanded position, normalised', -1., 1.)
pid_elevator_cmd = BoundedProperty('ap/elevator_cmd', 'elevator commanded position, normalised', -1., 1.)
...
velocity_roc=BoundedProperty('velocities/h-dot-fps','ROC',0,1000)
trim_cmd=Property('simulation/do_simple_trim','trim command')
heading_hold=Property('ap/heading_hold','heading_hold_switch')
altitude_hold=Property('ap/altitude_hold','heading_hold')
...
#disturbance
turbulence_type = Property('atmosphere/turb-type', 'turb-type')
Milspec_fps= Property('atmosphere/turbulence/milspec/windspeed_at_20ft_AGL-fps', 'turblence-speed')
Milspec_severity = Property('atmosphere/turbulence/milspec/severity','turblence-severity')
Psi_wind = Property('atmosphere/psiw-rad', 'wind-psi-rad')
wind_magnitude_north = Property('atmosphere/total-wind-north-fps','wind-magnitude')
wind_magnitude_east = Property('atmosphere/total-wind-east-fps', 'wind-magnitude')
wind_mgnitude_down = Property('atmosphere/total-wind-down-fps', 'wind-magnitude')
#battefield information
outborad = BoundedProperty('law/if-in-battlefield','if in battlefield 1 represent out 0 represent in', 0, 1)
blood_left = BoundedProperty('LifeCurrent', 'life blood left represent hurt',0, 100)
# oponent_blood_left = BoundedProperty('life/oponent-blood-left', 'oponent life blood left represent hurt',0, 100)
#weapon information
missile_left = BoundedProperty('GeneraMissileCurrentNum', 'missile left',0,6)
bullet_left = BoundedProperty('BulletCurrentNum', 'bullet-left',0,1000)
rodar_lock=BoundedProperty('isTargetLocked', 'missile lock target',0, 1)
been_locked = BoundedProperty('MissileAlert','been locked by the opponent',0,1)
#weapon control
weapon_launch = BoundedProperty('fcs/weapon-launch','1 represent launch missile 2 represent bullet 0 represent stay',0,2)
