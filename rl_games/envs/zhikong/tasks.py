...
from zhikong import assessors, rewards, utils
from zhikong.simulation import Simulation
from zhikong.properties import BoundedProperty, Property
from zhikong.aircraft import Aircraft
from zhikong.rewards import RewardStub
...
class Task(ABC):
    ...
    @abstractmethod
    def task_step(self, sim: Simulation, sim2: Simulation, action: Sequence[float], sim_steps: int) \
    ...
    @abstractmethod
    def observe_first_state(self, sim: Simulation, sim2 : Simulation) -> np.ndarray:
        """
...
class FlightTask(Task, ABC):
    ...
    """
    communication = comm_interface.env('127.0.1.1',8888,0)
    INITIAL_ALTITUDE_FT = 5000
    INITIAL_ALTITUDE_FT_2 = 5000
    ...
    base_initial_conditions = types.MappingProxyType(  # MappingProxyType makes dict immutable
        {prp.initial_altitude_ft: INITIAL_ALTITUDE_FT,
         ...
         prp.initial_longitude_geoc_deg: 0.01,
         prp.initial_latitude_geod_deg: 0.01  # corresponds to UoBath
         }
    )
    base_initial_conditions_2 = types.MappingProxyType(  # MappingProxyType makes dict immutable
        {prp.initial_altitude_ft: INITIAL_ALTITUDE_FT_2,
         prp.initial_terrain_altitude_ft: 0.00000001,
         prp.initial_longitude_geoc_deg: 0.01,
         prp.initial_latitude_geod_deg: 0.05  # corresponds to UoBath
         }
    )
    ...
    def _make_state_class(self) -> None:
        ...
        self.State2 = namedtuple('State2', legal_attribute_names)
        r_legal_attribute_names = [prop.get_legal_name() for prop in
                                 (self.state_variables+self.cost_variables)]
        #self.State = namedtuple('State', legal_attribute_names)
        self.Reward_state = namedtuple('reward_state',r_legal_attribute_names)
        self.Reward_state2 = namedtuple('reward_state', r_legal_attribute_names)



    def _accept_from_socket(self, msg_receive, sim: Simulation, sim2: Simulation):

        for key in msg_receive.keys():
            if key=='red':
                for key2 in msg_receive['red']['red_0']:
                    sim.substitution_dict[key2] = msg_receive['red']['red_0'][key2]
            if key == 'blue':
                for key2 in msg_receive['blue']['blue_0']:
                    sim2.substitution_dict[key2] = msg_receive['blue']['blue_0'][key2]



    def task_step(self, sim: Simulation, sim2: Simulation, action: Dict[str,float], sim_steps: int):
        # input actions
        cmd=self._lag_cmd2(action['p1'])
        cmd2=self._lag_cmd2(action['p2'])
        control_commands={'red':{'red_0':{}},'blue':{'blue_0':{}}}
        for prop, command, command2 in zip(self.action_variables, cmd, cmd2):
            sim[prop] = command
            sim2[prop]=command2
            control_commands['red']['red_0'][prop.name]=command
            control_commands['blue']['blue_0'][prop.name]=command2

        # run simulation
        # for _ in range(sim_steps):
        #     sim.run()
        #sim.run(control_commands)
        msg_recieve=self.communication.step(control_commands)
        self._accept_from_socket(msg_recieve,sim, sim2)
        self._update_custom_properties(sim,sim2)
        self.prev_action_variables = [cmd,cmd2]
        state1 = self.State(*(sim[prop] for prop in self.state_variables))
        state2 = self.State2(*(sim2[prop] for prop in self.state_variables))
        reward_state = self.Reward_state(*(sim[prop] for prop in (self.state_variables+self.cost_variables)))
        reward_state2 = self.Reward_state2(*(sim2[prop] for prop in (self.state_variables+self.cost_variables)))
        done1 = self._is_terminal(sim)
        done2 = self._is_terminal(sim2)
        done = done1 or done2
        reward1 = self.assessor.assess(reward_state, self.last_reward_state, done1)
        reward2 = self.assessor.assess(reward_state2, self.last_reward_state2, done2)
        if self.debug:
            if self._validate_state(state1, done1, action, reward1):
                done=True
            if self._validate_state(state2, done2, action, reward2):
                done=True
        if done:
            reward1 = self._reward_terminal_override(reward1, sim)
            reward2 = self._reward_terminal_override(reward2, sim2)
        self._store_reward(reward1, sim)
        self._store_reward(reward2,sim2)
        self.last_reward_state = reward_state
        self.last_reward_state2 = reward_state2
        info = {'reward': reward1, 'reward2':reward2}
        # print(sim[prp.turbulence_type])
        # print(sim[prp.Milspec_fps])
        # print(sim[prp.Wind_magnitude])
        # print(sim[prp.Psi_wind])
        return state1, state2, (reward1.agent_reward(),reward2.agent_reward()), done, info
    ...
    def _update_custom_properties(self, sim: Simulation,sim2: Simulation) -> None:
    ...
    def observe_first_state(self, sim: Simulation, sim2: Simulation):
        self._new_episode_init(sim,sim2)
        self._update_custom_properties(sim, sim2)
        ...
        self.prev_action_variables=np.zeros(len(self.action_variables))
        return np.array([state,state2]).flatten(), np.array(state), np.array(state2)

    def _new_episode_init(self, sim: Simulation, sim2: Simulation) -> None:
        """
        ...
        self._store_reward(RewardStub(1.0, 1.0), sim2)
    ...
    def _lag_cmd2(self, action):
        return self.clip(action, [-1, -1, -1, 0], [1, 1, 1, 1])

    def _start_wind_up(self, sim: Simulation):

        sim[prp.turbulence_type] = 3
        sim[prp.Milspec_fps] = 50
        sim[prp.Milspec_severity] =4

    def _end_wind_up(self, sim: Simulation):

        sim[prp.turbulence_type] = 3
        sim[prp.Milspec_fps] = 0
        sim[prp.Milspec_severity] = 0
...
class AircombatTask(FlightTask):
    ...
    initial_altitude_state= BoundedProperty('initial/altitude-error-ft',
                                        'error to desired altitude [ft]',
                                        prp.altitude_sl_ft.min,
                                        prp.altitude_sl_ft.max)
    oponent_blood_left = BoundedProperty('hurt/opponet-blood-left', 'the oponent blood left',0 ,100)

    input_cost=BoundedProperty('error/input-cost',
                                      'error to input cost', 0, 10000)
    EC_opponent_hurt: rewards.AsymptoticErrorComponent
    EC_self_blood: rewards.AsymptoticErrorComponent
    action_variables = (prp.aileron_cmd, prp.elevator_cmd, prp.rudder_cmd, prp.throttle_cmd, prp.weapon_launch)
    cost_variables=(prp.aileron_cmd, prp.elevator_cmd, prp.rudder_cmd, prp.throttle_cmd)
    ...
    def __init__(self, shaping_type: Shaping, step_frequency_hz: float, aircraft: Aircraft,
        ...
        """
        self.max_time_s = self.DEFAULT_EPISODE_TIME_S
        ...
        self.extra_state_variables = (prp.outborad, prp.rodar_lock, prp.been_locked, prp.sideslip_deg, prp.missile_left, prp.bullet_left,
                                     prp.blood_left, self.oponent_blood_left, self.steps_left)
    ...
    def _make_base_reward_components(self) -> Tuple[rewards.RewardComponent, ...]:
        ...
        self.EC_opponent_hurt=rewards.AsymptoticErrorComponent(name='opponent_hurt',
                                         prop=self.oponent_blood_left,
                                         state_variables=self.state_variables+self.cost_variables,
                                         target=0.0,
                                         is_potential_based=False,
                                         weight=0.5,
                                         scaling_factor=50)
        self.EC_self_blood=rewards.AsymptoticErrorComponent(name='self_blood',
                                         prop=prp.blood_left,
                                         state_variables=self.state_variables+self.cost_variables,
                                         target=100,
                                         is_potential_based=False,
                                         weight=0.2,
                                         scaling_factor=50)
        self.EC_velocity = rewards.AsymptoticErrorComponent(name='pilot_velocity',
                                                        prop=prp.u_fps,
                                                        state_variables=self.state_variables + self.cost_variables,
                                                        ...
                                                        weight=0.3,
                                                        scaling_factor=350)
        base_components = (self.EC_opponent_hurt,self.EC_self_blood,self.EC_velocity)
    ...
    def _select_assessor(self, base_components: Tuple[rewards.RewardComponent, ...],
        ...
        if shaping is Shaping.STANDARD:
            ...
            self.wings_level_ = rewards.AsymptoticErrorComponent(name='wings_level',
                                                           ...
                                                           weight=0.1,
                                                           scaling_factor=0.1)
            self.no_sideslip_ = rewards.AsymptoticErrorComponent(name='no_sideslip',
                                                           ...
                                                           weight=0.1,
                                                           scaling_factor=0.1)
            self.optimal_rudder_ = rewards.AsymptoticErrorComponent(name='ddd',
                                                           prop=prp.rudder_cmd,
                                                           state_variables=self.state_variables+self.cost_variables,
                                                           target=0.0,
                                                           is_potential_based=True,
            ...
            potential_based_components = (self.wings_level_, self.no_sideslip_, self.optimal_rudder_)
    ...
    def get_initial_conditions(self) -> Dict[Property, float]:
        #velocity=random.uniform(200,700)
    ...
    def get_initial_conditions_2(self) -> Dict[Property, float]:
        #velocity=random.uniform(200,700)
        extra_conditions = {prp.initial_u_fps: self.aircraft.get_cruise_speed_fps(),
                            prp.initial_v_fps: 0,
                            prp.initial_w_fps: 0,
                            ...
                            prp.initial_r_radps: 0,
                            prp.initial_roc_fpm: 0,
                            prp.initial_heading_deg: self.INITIAL_HEADING_DEG_2,
                            }
        return {**self.base_initial_conditions_2, **extra_conditions}

    def _update_custom_properties(self, sim: Simulation, sim2: Simulation) -> None:
        self._decrement_steps_left(sim)
        self._decrement_steps_left(sim2)
        self._opponent_blood_left(sim, sim2)
        self._opponent_blood_left(sim2, sim)


    def _opponent_blood_left(self, sim: Simulation, sim2:Simulation):
        sim[self.oponent_blood_left]=sim2[prp.blood_left]
        if sim2[prp.outborad]==1:
            sim[self.oponent_blood_left] = 0
            sim2[prp.blood_left] = 0
    ...
    def _is_terminal(self, sim: Simulation) -> bool:
        ...
        self_done = sim[prp.blood_left]<1
        ...
        return terminal_step or state_out_of_bounds or self._altitude_out_of_bounds(sim) or self_done

    def _altitude_out_of_bounds(self, sim: Simulation) -> bool:

        too_low=sim[prp.altitude_sl_ft] < prp.altitude_sl_ft.min+500
        too_high=sim[prp.altitude_sl_ft] > prp.altitude_sl_ft.max-500
    ...
    def _new_episode_init(self, sim: Simulation, sim2:Simulation) -> None:
        super()._new_episode_init(sim, sim2)
        sim[self.steps_left] = self.steps_left.max
        sim2[self.steps_left] = self.steps_left.max
        sim[self.oponent_blood_left] = 100
        sim2[self.oponent_blood_left] = 100
    ...
    def get_props_to_output(self) -> Tuple:
        return (prp.u_fps, prp.altitude_sl_ft, self.altitude_error_ft, self.target_track_deg,
                ...
                self.last_assessment_reward, self.steps_left,prp.lat_geod_deg,prp.lng_geoc_deg)
