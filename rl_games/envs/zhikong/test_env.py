...
config = {"ip": '127.0.1.1', "num_agents":4, "worker_index":888, "scenes":1,
          'excute_path': "/home/lzy/lzy/human-ai/Air/8-23/2.4/Linux/Mono/Linux/ZK.x86_64",
          "playmode": 0, "action_space_type": "MultiDiscrete", "red_agents_num":2,
          "blue_agents_num":2, "episode_max_length": 1000, "change_target": False}

env = AirCombatEnv(**config)
obs = env.reset(init=True)

# 1v1
# ego_actions = np.array([[4,4,4,4,1,0]], dtype=float)
# op_actions = np.array([[4,4,4,4,0,0]], dtype=float)

# 2v2
ego_actions = np.array([[4,4,4,4,1,0], [4,4,4,4,1,0]], dtype=float)
op_actions = np.array([[4,4,4,4,0,0], [4,4,4,4,0,0]], dtype=float)
