...
print(f"env initing...")
env=comm_interface.env('127.0.1.1',8888,0)
print(f"env define done")
# 设置发送初始化时的模式
...
# print(f"env reset done: {msg_reieve}")
# 测试加速倍速所用
...
while(1):
    ...
    # if num >= 9999:
    #     num = 0
    #     env.reset(dict_reset)
    print(f"current step: {num}")
    # print(f"副翼 state: {state['red']['red_0']['fcs/aileron-cmd-norm']}")
    # print(f"升降舵 state: {state['red']['red_0']['fcs/elevator-cmd-norm']}")
    # print(f"方向舵 state: {state['red']['red_0']['fcs/rudder-cmd-norm']}")
    # print(f"red0 真实速度 state: {state['red']['red_0']['velocities/ve-fps']}")
    # print(f"red:")
    # print(f"经度 state: {state['red']['red_0']['position/long-gc-deg']}")
    # print(f"纬度 state: {state['red']['red_0']['position/lat-geod-deg']}")
    # print(f"速度x state: {state['red']['red_0']['velocities/u-fps']}")
    # print(f"速度y state: {state['red']['red_0']['velocities/v-fps']}")
    # print(f"速度z state: {state['red']['red_0']['velocities/w-fps']}")
    # print(f"下方向速度 state: {state['red']['red_0']['velocities/v-down-fps']}")
    # print(f"航向角 state: {state['red']['red_0']['attitude/psi-deg']}")
    # print(f"侧滑角 state: {state['red']['red_0']['aero/beta-deg']}")

    # print(f"blue:")
    # for key in state.keys():
    #     for index in key.keys():
    #         pass
    print(state['red']['red_0'].keys())
    # print(f"state keys: {state.keys()}")
    1/0
    print(f"state {state}")
    # print(f"state key.keys: {state.keys()}")
    camp = state.keys()
    if not 'red' in state and 'blue' in state:
        print(f"blue win! reset")
        env.reset(dict_reset)
    elif  'red' in state and not 'blue' in state:
        print(f"red win! reset")
        env.reset(dict_reset)

    # if (state['red']['red_0']['DeathEvent'] !=99):
    #     print("重置")
    #     print(f"current step: {num}")
    #     env.reset(dict_reset)
    # print(state['blue'].keys())
    # if 'blue' in state.keys() and 'blue_0' in state['blue'].keys():
    #     print(f"blue0 真实速度 state: {state['blue']['blue_0']['velocities/ve-fps']}")
    #     print(f"经度 state: {state['blue']['blue_0']['position/long-gc-deg']}")
    #     print(f"纬度 state: {state['blue']['blue_0']['position/lat-geod-deg']}")
    #     print(f"速度x state: {state['blue']['blue_0']['velocities/u-fps']}")
    #     print(f"速度y state: {state['blue']['blue_0']['velocities/v-fps']}")
    #     print(f"速度z state: {state['blue']['blue_0']['velocities/w-fps']}")
    #     print(f"航向角 state: {state['blue']['blue_0']['attitude/psi-deg']}")
    #     print(f"侧滑角 state: {state['blue']['blue_0']['aero/beta-deg']}")
